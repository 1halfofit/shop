...
<script type="text/javascript">
    function addCart(){
        ...
        var pid = $("#hd").val();
        location.href="${pageContext.request.contextPath}/addCar?pid="+pid+"&buyNum="+buyNum;
    }
    ...
    function input(){
        ...
        if(buyNum<1){
            ...
            $("#buyNum").val(Math.floor($("#buyNum").val()))
        }
...
<body>
    ...
    <input type="hidden" value="${pro.pid}" id="hd">
	<div class="container">
		<div class="row">
			<div
			...
			<div style="margin: 0 auto; width: 950px;">
				...
				<div class="col-md-6">
					...
					<div style="margin: 10px 0 10px 0;">
						亿家价: <strong style="color: #ef0101;">￥：${pro.shopprice}元/份</strong> 参 考 价：
						<del>￥${pro.marketprice}元/份</del>
					</div>

					<div style="margin: 10px 0 10px 0;">
						促销: <a target="_blank" title="限时抢购 (2014-07-30 ~ 2015-01-01)"
							style="background-color: #f07373;">限时抢购</a>
							抢购时间 (2018-04-18 ~ 2018-04-30)
					</div>
					...
					<div
						...
						<div
							...
							<input type="button" value="-" onclick="lessen()">
						</div>
			...
			<div style="width: 950px; margin: 0 auto;">
				...
				<%--<div>
                    <c:forEach items="${pro.pdesc}" var="pd">
                        <img src="${pageContext.request.contextPath }/${pd.imagepath}">
                    </c:forEach>
				...
				<%--<div style="background-color: #d3d3d3; width: 900px;">
					<table class="table table-bordered">
						<tbody>
							...
							<c:if test="${empty pro.els}">
                                    <th>暂无商品评论信息 <a>[该商品暂无评论]</a></th>        
                                </c:if>
                                <c:if test="${!empty pro.els}">
                                <c:forEach items="${pro.els}" var="el">
							       <tr class="warning">
								         <th>${el.username}发表的评论:<br>  ${el.dis}</th>         												
							       </tr>
							 </c:forEach>
							</c:if>	
						</tbody>
